...
fn vs_main(@location(0) groundPos: vec2<f32>, @location(1) texel: vec2<f32>) -> VertexOutput {
  ...
  let terrainHeight = textureLoad(heightTex, coord, 0).r;
  let waterDepth = textureLoad(waterTex, coord, 0).r;
...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
  if (in.waterDepth <= 0.0) {
    discard;
  }
  // Arbitrary reference depth for the shallow->deep ramp — not physically
  // calibrated, just enough to read as water for M1. Revisit at M5.
