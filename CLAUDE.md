...
- **Headless GPU checks: `npm run test:gpu`** (`tools/gpu-check/`). Runs the real `src/sim/` modules on Dawn (the `webgpu` npm package; software Vulkan on milliwaysserver) with pass/fail checks: validation errors, mass conservation, no negatives, erosion/slumping behaving (sand 34°, earth 45°, piles come to rest). Run it after any sim shader change, before testing on-device. **Correctness only, not speed** (it showed 22 % where the iPad showed 6×), and it segfaults at fields ≥ 384², so scenarios stay ≤ 256². `npm test` (vitest) is the CPU unit tests.
