...
### P7 — Future relief theme idea: ridgeline elevation plot *(uploaded 2026-08-28, logged 2026-09-23)*
`docs/design/Example relief mapping.jpg` — a ridgeline-style ("joy plot") elevation map of the Levant: horizontal elevation profiles on black, coloured blue below sea level → white at sea level → orange → purple peaks. Third-party artwork (signed "Nicolas · @habibicode"), kept as inspiration only. **Confirmed by Dane 2026-09-23: an idea for a future relief theme.** Two separable pieces: (1) the *palette* — blue below sea level, white at sea level, warm ramp above — is a pure data change under D16's `ReliefTheme` system; (2) the *ridgeline rendering style* (horizontal elevation profiles on black) is not — it's a different rendering technique, not just a colour ramp, and would need its own pass or a shader mode. Consider at M5 alongside contour lines (P6), which share the "draw lines at elevation" machinery.
