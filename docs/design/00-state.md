...
| M4 | **In progress** — first slice (hydraulic erosion + sediment transport + dev panel) built 2026-09-25, see `06-m4-brief.md` §0. `06-m4-brief.md` (drafted 2026-09-02). Erosion / deposition, two-channel sediment transport, thermal slumping on top of the M3 pipe model. Completes the D15 authority shift (terrain → GPU-authoritative; **D23** pending). Erosion itself is behind a toggle, keep-or-cut undecided (**D22**). Mostly parameter tuning — a live dev panel is in scope from the first slice. |
