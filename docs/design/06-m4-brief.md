...
**`?demo=erode` built 2026-09-25** (`src/terrain/erodeDemo.ts`). Hand-built rather than a generator seed (a small departure from §3.1's "fixed seed"): fully deterministic, and shaped to hit both incision and delta criteria in one map — dry plateau (rock 50) → even slope, 60 m over 400 cells (~2.5°) → sea (level 0, shoreline ~col 660) deepening to the east edge; 3 m earth + 1 m sand everywhere; two octaves of ≤ 0.55 m rock roughness so the flow has something to channelise on. One spring at (col 320, row 512), 200/s; rain and evaporation off. Rain on (dev panel) should add tributaries. Not yet watched on-device.
